...
def backtest_partial_sl30(z_threshold=2.0, corr_threshold=0.8, SL=30, filename="trade_log_SL30_DD.csv"):
        ...
        if not in_trade:
            ...
            if exit_trade:
                ...
                trade_log.append({
                    ...
                    "equity": equity,
                    "tradeDD": worst_unreal  # DD ต่อไม้
                })
                in_trade = False
    ...
    print(f"Max DD (eq)  : {trades['drawdown'].min():.2f} pips")
    ...
    ax[1].plot(trades["exit"], trades["drawdown"], color="red", label="Drawdown")
    ax[1].set_title("Drawdown Curve")
    ax[1].set_ylabel("Pips")
    ax[1].legend()
