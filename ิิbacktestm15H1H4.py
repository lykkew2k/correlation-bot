...
# === Histogram กำไรต่อไม้ ===
def plot_pnl_hist(results, label=""):
    if results is None or results.empty:
        return
    plt.hist(results["net_pnl"], bins=50, alpha=0.6, label=label)

# เรียกใช้หลังจาก backtest ทุก TF
plt.figure(figsize=(10,5))
for tf in ["M15", "H1", "H4"]:
    eur, gbp = load_data(tf)
    results = backtest(eur, gbp, tf)
    plot_pnl_hist(results, tf)
